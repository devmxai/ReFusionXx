...
class Stage5MotionBlurCompositeView(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val centerMatrix = absoluteMatrixForSample(
            sample = samples[centerIndex],
            viewWidth = viewWidth,
            viewHeight = viewHeight,
        )
        val centerInverse = Matrix()
        if (!centerMatrix.invert(centerInverse)) {
            return
        }
        ...
        samples.forEachIndexed { index, sample ->
            ...
            val sampleMatrix = absoluteMatrixForSample(
                sample = sample,
                viewWidth = viewWidth,
                viewHeight = viewHeight,
            )
            sampleMatrix.postConcat(centerInverse)
    ...
    private fun absoluteMatrixForSample(
