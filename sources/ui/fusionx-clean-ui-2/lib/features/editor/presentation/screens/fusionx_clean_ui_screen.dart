...
class _FusionXCleanUiScreenState extends State<FusionXCleanUiScreen>
  ...
  static const List<AnimateBrowserItem> _manualTransitionFxItems =
      _scopedVideoFxItems;
  static const List<AnimateBrowserItem> _manualTransitionMotionModifierItems =
      <AnimateBrowserItem>[
    AnimateBrowserItem(
      id: 'motion_blur',
      label: 'Motion Blur',
      category: 'Motion Modifiers',
      summary:
          'Bind shutter blur to the selected Transform, Position, Scale, or Rotation motion channel.',
      keywords: <String>[
        'motion',
        'blur',
        'modifier',
  ...
  bool _tracksHavePlayableVideo(List<TimelineTrackData> tracks) {
    for (final track in tracks) {
      if (track.kind != TimelineTrackKind.video) {
        continue;
      }
  ...
  bool _tracksHaveVisualMedia(List<TimelineTrackData> tracks) {
    for (final track in tracks) {
      if (track.kind != TimelineTrackKind.video &&
          track.kind != TimelineTrackKind.image) {
        continue;
  ...
  CompositionMediaPlaybackProjectionResult
    ...
    final sceneScope = _sceneScopeSession;
    if (sceneScope != null) {
      return _compositionMediaPlaybackProjectionAdapter.projectSceneScope(
        sceneScope,
      );
    }
    final project = _motionProject;
    if (project == null || _sceneClips.isEmpty) {
      return CompositionMediaPlaybackProjectionResult(
        tracks: const <TimelineTrackData>[],
      );
    }
    return _compositionMediaPlaybackProjectionAdapter.projectRootComposition(
      project: project,
      sceneClips: _sceneClips,
    );
  }

  List<TimelineTrackData> _effectiveMediaPlaybackTracksFor(
    List<TimelineTrackData> tracks,
  ) {
    final scopedProjection = _sceneScopeSession == null
        ? null
        : _compositionMediaProjectionForCurrentScope();
    if (scopedProjection != null && scopedProjection.hasVisualMedia) {
      return scopedProjection.tracks;
    }
    if (_tracksHaveVisualMedia(tracks)) {
      return tracks;
    }
    final rootProjection = _compositionMediaProjectionForCurrentScope();
    if (rootProjection.hasVisualMedia) {
  ...
  void _refreshLiveScrubPreviewSourceCatalog({
    ...
    final descriptors = _buildLiveScrubPreviewSourceDescriptors(
      _effectiveMediaPlaybackTracksFor(tracks ?? _tracks),
    );
  ...
  void _handleTransportStateChanged() {
    ...
    final transitionFocusContext = transitionFocusSession == null
        ? null
        : _transitionFocusContextForSession(transitionFocusSession);
  ...
  TimelineTrackTransitionData _createSceneScopeTransitionForBoundary({
    ...
    final timing = _defaultTransitionTimingForPreset(preset);
  ...
  ({TimelineTime? leadingDurationTime, TimelineTime? trailingDurationTime})
    ...
    final leadingTicks = duration.inProjectTicks ~/ 2;
    final leading = TimelineTime.fromProjectTicks(leadingTicks);
    return (
      leadingDurationTime: leading,
      trailingDurationTime: duration - leading,
    );
  }
  ...
  Future<void> _openSceneScopeTransitionInspector(
    ...
    switch (result.action) {
      ...
      case TransitionInspectorAction.openManual:
        final manualTransition = result.transition.copyWith(
  ...
  Future<void> _syncVideoTimelineTransport({
    ...
    final effectiveTracks = _effectiveMediaPlaybackTracksFor(tracks);
    final videoTrackIndex = effectiveTracks.indexWhere(
      (track) => track.kind == TimelineTrackKind.video,
    );
    if (videoTrackIndex < 0) {
    ...
    for (final clip in effectiveTracks[videoTrackIndex].clips) {
      final asset = _assetForId(clip.assetId);
      final sourceUri = asset?.sourceUri;
  ...
  _TransitionFocusContext? _transitionFocusContextForSession(
    _TransitionFocusSession session,
  ) {
    return _transitionFocusContextById(
      session.transitionId,
      sourceSceneId: session.sourceSceneId,
    );
  }

  _TransitionFocusContext? _transitionFocusContextById(
    String transitionId, {
    String? sourceSceneId,
  }) {
    final sceneScope = sourceSceneId == null ? null : _sceneScopeSession;
    final TimelineTrackData? track;
    final TimelineTime scopeDurationTime;
    if (sceneScope != null && sceneScope.sourceSceneId == sourceSceneId) {
      final sceneTracks = _buildSceneScopeTracks(sceneScope);
      TimelineTrackData? sceneVideoTrack;
      for (final candidate in sceneTracks) {
        if (candidate.kind == TimelineTrackKind.video) {
          sceneVideoTrack = candidate;
          break;
        }
      }
      track = sceneVideoTrack;
      scopeDurationTime = sceneScope.localRange.duration;
    } else if (sourceSceneId != null) {
      return null;
    } else {
      final videoTrack = _tracks.where(
        (candidate) => candidate.kind == TimelineTrackKind.video,
      );
      if (videoTrack.isEmpty) {
        return null;
      }
      track = videoTrack.first;
      scopeDurationTime = _timelineDurationTime;
    }
    if (track == null) {
      return null;
    }
    ...
    final contextSeamTime =
        sceneScope == null ? seamTime : sceneScope.localToRoot(seamTime);
    final contextActiveStartTime = sceneScope == null
        ? activeStartTime
        : sceneScope.localToRoot(activeStartTime);
    final contextActiveEndTime = sceneScope == null
        ? activeEndTime
        : sceneScope.localToRoot(activeEndTime);
    final contextStartTime =
        sceneScope == null ? startTime : sceneScope.localToRoot(startTime);
    final contextEndTime =
        sceneScope == null ? endTime : sceneScope.localToRoot(endTime);
    return _TransitionFocusContext(
      transition: transition,
      leftClip: leftClip.clip,
      rightClip: rightClip.clip,
      seamTime: contextSeamTime,
      activeStartTime: contextActiveStartTime,
      activeEndTime: contextActiveEndTime,
      startTime: contextStartTime,
      endTime: contextEndTime,
    );
  }

  TimelineTrackTransitionData? _transitionFocusTransitionById(
    String transitionId, {
    String? sourceSceneId,
  }) {
    if (sourceSceneId == null) {
      return _videoTrackTransitionById(transitionId);
    }
    return _transitionFocusContextById(
      transitionId,
      sourceSceneId: sourceSceneId,
  ...
  void _upsertTransitionFocusTransition(
    TimelineTrackTransitionData transition, {
    String? sourceSceneId,
  }) {
    if (sourceSceneId == null) {
    ...
    _upsertSceneScopeTransition(sceneScope, videoTrack, transition);
  }
  ...
  double _motionBlurSettingFallback(String settingId) {
  ...
  bool _isTransitionMotionBlurLaneId(String laneId) {
    return laneId == 'motion_blur' ||
        _transitionMotionBlurModifierLaneIds.contains(laneId);
  }

  bool _canAddTransitionMotionModifierToLane(String laneId) {
    return laneId == 'position' ||
        laneId == 'scale' ||
        laneId == 'rotation' ||
        laneId == 'transform';
  }

  String _transitionMotionBlurLaneIdForParent(String parentLaneId) {
    return switch (parentLaneId) {
      'position' => 'position.motion_blur',
      'scale' => 'scale.motion_blur',
      'rotation' => 'rotation.motion_blur',
      _ => 'transform.motion_blur',
    };
  }
  ...
  String _motionBlurSettingControlKey(String laneId, String controlId) {
    final legacySettingKey = switch (controlId) {
      'motionBlurShutterAngle' => 'motion_blur_shutter_angle',
      'motionBlurShutterPhase' => 'motion_blur_shutter_phase',
      'motionBlurSamples' => 'motion_blur_samples',
      'motionBlurAdaptiveSamples' => 'motion_blur_adaptive_samples',
      'motionBlurMaxTrail' => 'motion_blur_max_trail',
      _ => '',
    };
    if (legacySettingKey.isEmpty) {
      return '';
    }
  ...
  TimelineAnimationLaneData _normalizeTransitionFocusManualLane(
    ...
    final migratedLane = lane.id == 'motion_blur_amount'
        ? lane.copyWith(id: 'motion_blur', label: 'Motion Blur')
        : lane;
  ...
  TimelineTrackTransitionData _normalizeTransitionFocusManualTransition(
    ...
    var changed = false;
    final migratedEffectIds = <String>[];
    for (final effectId in transition.manualEffectIds) {
      final migratedId =
          _legacyManualMotionBlurSettingLaneIds.contains(effectId)
              ? 'motion_blur'
              : effectId;
      if (!migratedEffectIds.contains(migratedId)) {
        migratedEffectIds.add(migratedId);
      }
      changed = changed || migratedId != effectId;
    }
    final migratedParameters = <String, double>{...transition.parameterValues};
    final normalizedLanes = <TimelineAnimationLaneData>[];
    for (final lane in transition.manualAnimationLanes) {
      if (_legacyManualMotionBlurSettingLaneIds.contains(lane.id) &&
          lane.id != 'motion_blur_amount') {
        final values = lane.alignedKeyframeValues(
          fallbackValue: _motionBlurSettingFallback(lane.id),
          clampToPercent: false,
        );
        if (values.isNotEmpty) {
          migratedParameters[lane.id] = values.first;
        }
        changed = true;
        continue;
      }
      ...
      normalizedLanes.add(normalized);
    }
    if (!changed) {
      return transition;
    }
    return transition.copyWith(
      manualEffectIds: List<String>.unmodifiable(migratedEffectIds),
      parameterValues: Map<String, double>.unmodifiable(migratedParameters),
  ...
  static const Map<String, _TransitionFocusLaneSpec> _transitionLaneLibrary =
    ...
    'motion_blur': _TransitionFocusLaneSpec(
      ...
      label: 'Motion Blur',
      editorDescription:
          'Applies source/layer Motion Blur as a general FX lane.',
      min: 0.0,
      max: 100.0,
      fallback: 0.0,
    ...
    'transform.motion_blur': _TransitionFocusLaneSpec(
      id: 'transform.motion_blur',
      groupLabel: 'Motion Modifier',
      label: 'Transform Motion Blur',
      editorDescription:
          'Binds shutter blur to the combined Position, Scale, and Rotation animation stack.',
      min: 0.0,
      max: 100.0,
      fallback: 0.0,
      tint: Color(0xFFFFB86B),
    ...
    'position.motion_blur': _TransitionFocusLaneSpec(
      id: 'position.motion_blur',
      groupLabel: 'Motion Modifier',
      label: 'Position Motion Blur',
      editorDescription:
          'Binds linear shutter blur to authored Position keyframe movement.',
      min: 0.0,
      max: 100.0,
      fallback: 0.0,
      tint: Color(0xFFFFD98A),
    ...
    'scale.motion_blur': _TransitionFocusLaneSpec(
      id: 'scale.motion_blur',
      groupLabel: 'Motion Modifier',
      label: 'Scale Motion Blur',
      editorDescription:
          'Binds radial shutter blur to authored Scale keyframe movement.',
      min: 0.0,
      max: 100.0,
      fallback: 0.0,
      tint: Color(0xFF8DFFAE),
    ...
    'rotation.motion_blur': _TransitionFocusLaneSpec(
      id: 'rotation.motion_blur',
      groupLabel: 'Motion Modifier',
      label: 'Rotation Motion Blur',
      editorDescription:
          'Binds circular shutter blur to authored Rotation keyframe movement.',
      min: 0.0,
      max: 100.0,
      fallback: 0.0,
      tint: Color(0xFFB79CFF),
  ...
  static const Set<String> _legacyManualMotionBlurSettingLaneIds = <String>{
    'motion_blur_amount',
  ...
  static const Set<String> _transitionMotionBlurModifierLaneIds = <String>{
  ...
  void _enterTransitionFocusMode(
    ...
    var context = _transitionFocusContextById(
      transitionId,
      sourceSceneId: sourceSceneId,
    );
    if (context == null) {
    ...
    if (context.transition.preset == TimelineTransitionPreset.manual) {
      ...
      if (normalizedTransition != context.transition) {
        ...
        context = _transitionFocusContextById(
          transitionId,
          sourceSceneId: sourceSceneId,
        );
        if (context == null) {
  ...
  Future<void> _openTransitionFocusAddMenu(String transitionId) async {
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: const Text('Motion Modifiers'),
                subtitle: const Text(
                  'Attach FX to Transform, Position, Scale, or Rotation motion.',
                ),
                onTap: () => Navigator.of(sheetContext).pop('modifier'),
              ),
              ListTile(
                leading: const Icon(
                  Icons.code_rounded,
    ...
    switch (action) {
      ...
      case 'fx':
        ...
        break;
      case 'modifier':
        await _openManualTransitionMotionModifierBrowser(
          transitionId,
          _transitionFocusSession?.selectedLaneId ?? 'transform',
        );
        break;
      case 'script':
        _showStageMessage(
  ...
  Future<void> _openManualTransitionMotionModifierBrowser(
    String transitionId,
    String parentLaneId,
  ) async {
    final effectiveParentLaneId =
        _canAddTransitionMotionModifierToLane(parentLaneId)
            ? parentLaneId
            : 'transform';
    setState(() {
      _isAnimateBrowserOpen = true;
    ...
    final item = await showModalBottomSheet<AnimateBrowserItem>(
      ...
      builder: (context) => MediaQuery.removeViewInsets(
        ...
        removeBottom: true,
        child: const AnimateBrowserBottomSheet(
          items: _manualTransitionMotionModifierItems,
        ),
      ),
    ).whenComplete(() {
      if (!mounted) {
    ...
    _addManualTransitionMotionModifier(
      transitionId: transitionId,
      parentLaneId: effectiveParentLaneId,
      item: item,
    );
  }
  ...
  void _addManualTransitionEffect(
    ...
    final requestedLaneId = item.id;
    if (_isTransitionMotionBlurLaneId(requestedLaneId)) {
      for (final laneId in <String>[
        ...
        if (_isTransitionMotionBlurLaneId(laneId) &&
          ...
          _showStageMessage(
            'Motion Blur already exists as ${_transitionLaneLibrary[laneId]?.label ?? 'a modifier'}.',
          );
          _selectTransitionFocusLane(laneId);
          return;
        }
      }
    }
    if (transition.manualEffectIds.contains(requestedLaneId)) {
      _showStageMessage('${item.label} already exists on this transition.');
      _selectTransitionFocusLane(requestedLaneId);
      return;
    }
    final baseSpec = _transitionLaneLibrary[requestedLaneId];
    if (baseSpec == null) {
      return;
    }
    final focusContext = _transitionFocusContextById(
      transitionId,
      sourceSceneId: sourceSceneId,
    );
    ...
    if (!existingLaneIds.contains(requestedLaneId)) {
    ...
    _updateTransitionFocusTransition(
      ...
      update: (current) => current.copyWith(
        ...
        parameterValues: <String, double>{
          ...current.parameterValues,
          requestedLaneId: current.parameterValue(
            requestedLaneId,
            fallback: baseSpec.fallback,
          ),
          if (requestedLaneId == 'motion_blur') ...<String, double>{
            'motion_blur_shutter_angle': current.parameterValue(
              'motion_blur_shutter_angle',
              fallback: _motionBlurSettingFallback(
                'motion_blur_shutter_angle',
              ),
            ),
            'motion_blur_shutter_phase': current.parameterValue(
              'motion_blur_shutter_phase',
              fallback: _motionBlurSettingFallback(
                'motion_blur_shutter_phase',
              ),
            ),
            'motion_blur_samples': current.parameterValue(
              'motion_blur_samples',
              fallback: _motionBlurSettingFallback('motion_blur_samples'),
            ),
            'motion_blur_adaptive_samples': current.parameterValue(
              'motion_blur_adaptive_samples',
              fallback:
                  _motionBlurSettingFallback('motion_blur_adaptive_samples'),
            ),
            'motion_blur_max_trail': current.parameterValue(
              'motion_blur_max_trail',
  ...
  void _addManualTransitionMotionModifier({
    required String transitionId,
    required String parentLaneId,
    required AnimateBrowserItem item,
  }) {
    if (item.id != 'motion_blur') {
      return;
    }
    final sourceSceneId = _transitionFocusSession?.transitionId == transitionId
        ? _transitionFocusSession?.sourceSceneId
    ...
    final requestedLaneId = _transitionMotionBlurLaneIdForParent(parentLaneId);
    String? existingMotionBlurLane;
    for (final laneId in <String>[
      ...transition.manualEffectIds,
      for (final lane in transition.manualAnimationLanes) lane.id,
    ]) {
      if (_isTransitionMotionBlurLaneId(laneId)) {
        existingMotionBlurLane = laneId;
        break;
      }
    }
    if (existingMotionBlurLane != null &&
        existingMotionBlurLane != requestedLaneId) {
      _showStageMessage(
        'Motion Blur already exists as ${_transitionLaneLibrary[existingMotionBlurLane]?.label ?? 'a modifier'}.',
      );
      _selectTransitionFocusLane(existingMotionBlurLane);
      return;
    }
    if (transition.manualEffectIds.contains(requestedLaneId)) {
      _showStageMessage('Motion Blur already exists on this motion channel.');
      _selectTransitionFocusLane(requestedLaneId);
      return;
    }
    final baseSpec = _transitionLaneLibrary[requestedLaneId];
    ...
    final existingLanes = transition.manualAnimationLanes;
    final nextLanes = <TimelineAnimationLaneData>[...existingLanes];
    if (!nextLanes.any((lane) => lane.id == requestedLaneId)) {
      final nextLane = _emptyTransitionFocusManualLane(
        laneId: requestedLaneId,
        targetClipId: transition.leftClipId,
      );
      if (nextLane != null) {
        final parentIndex =
            nextLanes.indexWhere((lane) => lane.id == parentLaneId);
        final insertIndex =
            parentIndex >= 0 ? parentIndex + 1 : nextLanes.length;
        nextLanes.insert(insertIndex, nextLane);
      }
    }
    _updateTransitionFocusTransition(
      transitionId,
      update: (current) {
        final nextLaneIds = <String>[
          ...
          if (!current.manualEffectIds.contains(requestedLaneId))
            requestedLaneId,
        ];
        return current.copyWith(
          ...
          parameterValues: <String, double>{
            ...
            for (final legacySettingKey in <String>[
              'motion_blur_shutter_angle',
              'motion_blur_shutter_phase',
              'motion_blur_samples',
              'motion_blur_adaptive_samples',
              ...
              _motionBlurSettingKeyForLane(requestedLaneId, legacySettingKey):
                  current.parameterValue(
                _motionBlurSettingKeyForLane(requestedLaneId, legacySettingKey),
                fallback: _motionBlurSettingFallback(legacySettingKey),
              ),
          },
          manualAnimationLanes: List<TimelineAnimationLaneData>.unmodifiable(
            nextLanes,
  ...
  List<LayerScopeValueControlSpec>? _transitionFocusValueControlsForSelection(
    _TransitionFocusContext focusContext,
    TimelineAnimationLaneData lane,
    int keyframeIndex,
  ) {
    if (keyframeIndex < 0 ||
    ...
    if (_isTransitionMotionBlurLaneId(lane.id)) {
      final transition = _transitionFocusTransitionById(
      ...
      double settingValue(String legacySettingKey) {
        return parameterValues[_motionBlurSettingKeyForLane(
              lane.id,
              legacySettingKey,
            )] ??
            _motionBlurSettingFallback(legacySettingKey);
      }
      ...
      return <LayerScopeValueControlSpec>[
        ...
        LayerScopeValueControlSpec(
          ...
          label: 'Shutter Angle',
          value: settingValue('motion_blur_shutter_angle'),
          min: 0.0,
          max: 720.0,
          divisions: 720,
          formatValue: _formatTransitionDegrees,
        ),
        LayerScopeValueControlSpec(
          id: 'motionBlurShutterPhase',
          label: 'Shutter Phase',
          value: settingValue('motion_blur_shutter_phase'),
          min: -360.0,
          max: 360.0,
          divisions: 720,
          formatValue: _formatTransitionDegrees,
        ...
        LayerScopeValueControlSpec(
          ...
          label: 'Samples',
          value: settingValue('motion_blur_samples'),
          min: 1.0,
          max: 32.0,
          divisions: 31,
          formatValue: _formatTransitionWholeNumber,
        ),
        LayerScopeValueControlSpec(
          id: 'motionBlurAdaptiveSamples',
          label: 'Adaptive Samples',
          value: settingValue('motion_blur_adaptive_samples'),
          min: 1.0,
          max: 64.0,
          divisions: 63,
          formatValue: _formatTransitionWholeNumber,
        ...
        LayerScopeValueControlSpec(
          id: 'motionBlurMaxTrail',
          label: 'Max Trail',
          value: settingValue('motion_blur_max_trail'),
          min: 0.0,
          max: 600.0,
  ...
  void _handleTransitionFocusValueControlChanged(
    ...
    final resolvedLaneId =
        controlId == 'rotationAngle' || controlId == 'rotationLoops'
            ? 'rotation'
            : controlId == 'motionBlurAmount' ||
                    motionBlurSettingControlIds.contains(controlId)
                ? (selectedLaneId != null &&
    ...
    if (motionBlurSettingControlIds.contains(controlId)) {
      final settingKey = _motionBlurSettingControlKey(lane.id, controlId);
  ...
  List<UniversalMotionChannelCollectionSource>
    ...
    if (unifiedSession != null) {
      sources.add(
        UniversalMotionChannelCollectionSource(
          id: 'unified_transition_scope_graph_bundle',
          channels: unifiedSession.graphBundle.channels,
        ),
      );
    }
  ...
  String _professionalVideoTransitionMode({
    ...
    return switch (mode) {
      TrueFrameRenderBackendMode.preview => 'preview',
      TrueFrameRenderBackendMode.liveScrub => 'liveScrub',
      TrueFrameRenderBackendMode.playback => 'playback',
      TrueFrameRenderBackendMode.export => 'export',
    };
  }
                            ...
                            child: Padding(
                              ...
                              child: transitionFocusContext != null
                                  ? TimelinePanel(
                                      ...
                                      onAnimationLaneModifierAdd: (laneId) =>
                                          _openManualTransitionMotionModifierBrowser(
                                        transitionFocusContext.transition.id,
                                        laneId,
                                      ),
                                      animationModifierSourceLaneIds: const <String>{
                                        'position',
                                        'scale',
                                        'rotation',
                                      },
