...
void main() {
  const sourcePath = 'assets/scene_programs/smart_test_app_prompt_scene.json';
  ...
  test('smart test app prompt preserves hold-frame render truth contracts',
    ...
    final sendIcon = _nodeBySourceElementId(evaluation, 'sendIcon');
    final text = _nodeBySourceElementId(evaluation, 'promptText');

    expect(shell.visible, isTrue);
    expect(shell.viewportBounds.width, greaterThan(620));
    expect(shell.viewportBounds.width, lessThan(840));
    ...
    expect(shell.viewportBounds.contains(sendIcon.viewportBounds), isTrue);
