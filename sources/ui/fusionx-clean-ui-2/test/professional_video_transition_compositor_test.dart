...
void main() {
  ...
  test('method channel render graph execution owns ordered graph only',
      () async {
    const channel = MethodChannel(
        'com.refusion.app/professional_video_transition_compositor');
    final messenger =
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planRenderGraphExecution');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
        ...
        'passExecutionStates': <Map<String, Object?>>[
          <String, Object?>{
            'passId': 'output-pass',
            'type': 'composeToTransitionSurface',
            'role': 'output',
            'index': 6,
            'inputs': <String>['transition-pass'],
    ...
    expect(result.canPlan, isTrue);
    expect(result.canOwnGraph, isTrue);
    expect(result.canExecuteGraph, isFalse);
    expect(result.drawsPixels, isFalse);
    expect(result.graphExecutorImplemented, isTrue);
    expect(result.rendererImplemented, isFalse);
    expect(result.graphOrderValid, isTrue);
    expect(result.graphDependenciesValid, isTrue);
    expect(result.executionOrder.last, 'output-pass');
    expect(result.requiredPassTypes.last, 'composeToTransitionSurface');
    expect(result.passExecutionStates.single.passId, 'output-pass');
    expect(result.passExecutionStates.single.readyForExecutor, isTrue);
    expect(
      result.blockedReasons,
      contains('native_transition_render_graph_executor_renderer_missing'),
    );
  });

  test('method channel surface renderer attaches graph without pixels',
      () async {
    const channel = MethodChannel(
        'com.refusion.app/professional_video_transition_compositor');
    final messenger =
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planSurfaceRenderer');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
    ...
    expect(result.canPlan, isTrue);
    expect(result.canAttachSurface, isTrue);
    expect(result.canRenderSurface, isFalse);
    expect(result.surfaceRendererImplemented, isTrue);
    expect(result.rendererImplemented, isFalse);
  ...
  test(
      'method channel renderer backend attaches command buffer before draw loop',
      () async {
    const channel = MethodChannel(
        'com.refusion.app/professional_video_transition_compositor');
    final messenger =
    ...
    expect(result.commandBufferReady, isTrue);
    expect(result.outputSurfaceAttached, isTrue);
    expect(result.backendReady, isTrue);
    expect(result.outputTarget, 'nativeTransitionCanvasSurface');
  ...
  test(
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planRendererDrawLoop');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
        ...
        'drawSubmissions': <Map<String, Object?>>[
          <String, Object?>{
    ...
    expect(result.canPlan, isTrue);
    expect(result.backendReady, isTrue);
    expect(result.drawLoopImplemented, isTrue);
    expect(result.shaderEvaluatorImplemented, isFalse);
  ...
  test('method channel transition pixel renderer binds pixel workload',
      () async {
    const channel = MethodChannel(
        'com.refusion.app/professional_video_transition_compositor');
    final messenger =
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planTransitionPixelRenderer');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
        ...
        'pixelInputCount': 1,
        'pixelInputs': <Map<String, Object?>>[
          <String, Object?>{
            'pixelInputId': 'pixel-input:0',
            'shaderInputId': 'shader-input:0',
            'submissionId': 'draw-submission:0',
            'commandId': 'command:output:0',
            'passId': 'output-pass',
    ...
    expect(result.pixelInputCount, 1);
    expect(result.pixelInputs.single.inputBound, isTrue);
    expect(result.pixelRendererImplemented, isFalse);
    expect(result.pixelRendererReady, isFalse);
  ...
  test(
      'method channel transition pixel frame buffer writer rejects still frames',
      () async {
    const channel = MethodChannel(
        'com.refusion.app/professional_video_transition_compositor');
    final messenger =
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planTransitionPixelFrameBufferWriter');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
        ...
        'transitionPixelFrameBufferWriterId':
            'transition-session:zoom-native-1:pixel-frame-buffer-writer:10000',
        'pixelProgramId':
            'transition-session:zoom-native-1:pixel-program:zoomInCamera',
        'outputSurfaceId':
            'transition-session:zoom-native-1:surface:transition-output:10000',
    ...
    expect(result.canPlan, isTrue);
    expect(result.writerBoundToFrameBuffer, isTrue);
    expect(result.requiresTemporalSamples, isTrue);
    expect(result.requiresDualSourceSamples, isTrue);
    expect(result.allowsStillFrameWrite, isFalse);
    expect(result.allowsSyntheticPixels, isFalse);
    expect(result.allowsPosterFrame, isFalse);
    expect(result.allowsThumbnailFallback, isFalse);
    expect(result.allowsBoundaryFreeze, isFalse);
    expect(result.writerReady, isFalse);
    ...
    expect(
      result.blockedReasons,
      contains('native_transition_pixel_frame_buffer_temporal_pixels_missing'),
    );
  });
  ...
  test('method channel transition pixel render execution blocks missing output',
    ...
    messenger.setMockMethodCallHandler(channel, (call) async {
      expect(call.method, 'planTransitionPixelRenderExecution');
      final arguments = call.arguments! as Map<Object?, Object?>;
      expect(arguments['definitionId'], 'zoomInCamera');
      expect(arguments['timelineTimeMs'], 10000);
      return <String, Object?>{
    ...
    expect(
      result.blockedReasons,
      contains('native_transition_pixel_output_missing'),
    );
  });
  ...
  test(
      ...
      return <String, Object?>{
        ...
        'outputs': <Map<String, Object?>>[
          <String, Object?>{
            ...
            'interactiveSurfaceFrameDelivered': false,
            'interactiveSurfaceFrameByteCount': 0,
    ...
    expect(
      result.outputs
          .every((output) => !output.interactiveSurfaceFrameDelivered),
      isTrue,
    );
    expect(
    ...
    final outputs = <ProfessionalVideoTransitionParityOutput>[
      for (final mode in <String>['preview', 'liveScrub', 'playback'])
        ProfessionalVideoTransitionParityOutput(
          ...
          outputSurfaceEndpointId: 'offscreen-proof-surface',
          outputSurfaceEndpointKind: 'offscreenNativeProofSurface',
          interactiveSurfaceId: 'interactive-$mode',
