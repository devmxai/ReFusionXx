...
- Composition Workspace W7 native live-decode window gate: Android dual-video decoder planning now separates exact sample decode probes from continuous video-stream readiness. Each outgoing/incoming decoder track records the required live decode timeline/source window, requires outgoing to cover the leading seam window and incoming to cover the trailing seam window, and blocks with `native_dual_video_live_decode_not_ready` until both roles can prove continuous sample coverage. This prevents any future transition renderer from treating decoded shutter samples or boundary frames as a playing video transition.
