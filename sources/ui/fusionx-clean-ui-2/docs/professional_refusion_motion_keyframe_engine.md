...
- Imported scene channels can be projected into normal editable timeline lanes.
...
1. Add a Layer Scope adapter that reads an existing element's graph channels and
   ...
   - Current status: `LayerScopeMotionProjectionAdapter` exists in
     `lib/features/editor/presentation/services/layer_scope_motion_projection_adapter.dart`.
     It is opacity-first, filters by selected element identity, preserves stable
     keyframe IDs, reports unsupported property issues, and is intentionally not
     wired to UI yet.
