...
- Composition Workspace W7 Zoom In Pro experiment gate: product testing may temporarily expose `Zoom In Pro` in the transition preset picker before the full native transition compositor is complete. This preset is explicitly marked as an experiment, transforms only the live native preview surface, and must not draw thumbnails, frozen boundary frames, decorative speed lines, Gaussian transition blur, or timeline-area overlays. It does not satisfy the professional Zoom In Camera contract, does not unlock other presets, and must not be documented as production transition support. The final professional path remains the native dual-video compositor with temporal shutter motion blur, mirror-edge tiling, preview/Live Scrub/playback parity, and later export parity.
