...
- Composition Workspace W7 professional transition readiness preflight: `ProfessionalVideoTransitionReadinessPreflight` now runs the complete compositor-readiness chain from capabilities through render session, source binding, frame samples, exact decode, dual decoder, temporal accumulation, mirror-edge tiling, render-pass graph, output surface, and preview/scrub/playback/export parity. It reports the exact blocking stages instead of allowing UI or future agents to infer readiness from one partial endpoint. This remains domain infrastructure and does not unlock any transition preset.
...
- Composition Workspace W7 native transition frame-sample contract: Android now exposes `planFrameSamples` on the professional video transition compositor channel. It reuses the strict render-session parser, rejects timeline times outside the transition window, maps the requested timeline frame to real outgoing/incoming source times, and emits temporal shutter sample timeline/source times from the render plan's motion blur policy. This is a renderer contract only: it still draws nothing and exposes no transition preset until native preview, Live Scrub, playback, and export parity are complete.
...
- Composition Workspace W7 native transition parity-output contract: Android now exposes `planParityOutputs` on the professional compositor channel. It requires preview, Live Scrub, playback, and future export to share the same native transition output contract and keeps modes blocked until the concrete renderer can render without fallback divergence.
