...
Professional Canva layer unification, video-as-layer truth, Manual Transition
keyframe time truth, and renderer parity for video/image/text/shape must also
follow:
...
- TRUEFRAME Core Phase B handoff safety: `FusionXCleanUiScreen` now suppresses Stage5 for manual temporal blur only after `ProfessionalVideoTransitionSurfaceOverlay` reports first-frame presentation for the specific `surfaceId + mode` key. `professional_video_transition_surface.dart` now emits explicit presentation callbacks, and the gate is covered by `test/universal_motion_engine_guard_test.dart`.
- TRUEFRAME Core Phase C universal target resolution slice: `TrueFrameUniversalTargetResolver` exists in `lib/features/editor/domain/services/trueframe_universal_target_resolver.dart` with tests in `test/trueframe_universal_target_resolver_test.dart`. It resolves canonical targets for project/scene/layer/element plus transition roles (`outgoing`/`incoming`), scene-clip instances, and groups with explicit blockers/diagnostics.
- TRUEFRAME Core Phase D graph projection slice: `TrueFrameExecutionGraphAdapter` and `TrueFrameExecutionGraph` models now exist in `lib/features/editor/domain/services/trueframe_execution_graph_adapter.dart` and `lib/features/editor/domain/models/trueframe_execution_graph_models.dart`, with tests in `test/trueframe_execution_graph_adapter_test.dart`. The adapter projects `MasterRenderGraph` into a core execution graph without introducing parallel truth, preserves node/binding topology, and enforces manual transition slice blockers for missing outgoing/incoming bindings or missing transition nodes.
- TRUEFRAME Core Phase E runtime evaluator slice: `TrueFrameCoreRuntimeEvaluator` and runtime frame-state models now exist in `lib/features/editor/domain/services/trueframe_core_runtime_evaluator.dart` and `lib/features/editor/domain/models/trueframe_runtime_evaluator_models.dart`, with tests in `test/trueframe_core_runtime_evaluator_test.dart`. The evaluator reads `TrueFrameExecutionGraph`, produces deterministic composition/node/effect states, and builds quality-aware `CoreMotionBlurSamplingPlan` objects from graph temporal blur nodes as the core runtime truth layer for the manual transition slice.
- TRUEFRAME Core Phase F backend adapter slice: `TrueFrameRenderBackend` now exists in `lib/features/editor/domain/services/trueframe_render_backend.dart` with tests in `test/trueframe_render_backend_test.dart`, and `FusionXCleanUiScreen` now routes manual transition preview/liveScrub/playback surface ownership through this contract. Stage5 remains presenter/fallback while manual temporal blur waits for the first professional frame, and suppression decisions are no longer hardcoded inline.
- TRUEFRAME Core Phase G motion blur runtime slice: manual transition temporal blur plan generation in `FusionXCleanUiScreen` now requests sampling from `TrueFrameCoreRuntimeEvaluator.buildSamplingPlan(...)` through projected TrueFrame graph state, then composes A/B contribution payloads from the same sample times. Legacy local offset fallback has been removed; core sampling is the only allowed source.
- TRUEFRAME Core Phase H export adapter slice: export composition building now emits a typed `ExportTrueFrameExecutionContract` from the same manual transition TrueFrame planning path (`FusionXCleanUiScreen`), includes it in export bridge payload metadata (`export_composition_models.dart`), and Stage6 preflight now consumes/validates this contract for motion-transition exports (`Stage6ExportManager.kt`) to keep export on the same graph/evaluator truth contract.
- TRUEFRAME Core Phase I layer-family expansion foundation: `TrueFrameExecutionGraphAdapter` now expands projected graph nodes with explicit `videoLayer`, `imageLayer`, `textLayer`, and `shapeLayer` families (plus compatibility stubs for `groupPrecomp`, `sceneClipInstance`, and `adjustmentControl`) without introducing a second engine. Projection tests (`test/trueframe_execution_graph_adapter_test.dart`) and guard checks (`test/universal_motion_engine_guard_test.dart`) now lock this Phase I core-node expansion path.
- TRUEFRAME Core Phase I runtime layer-family state: `TrueFrameCoreRuntimeEvaluator` now emits `resolvedLayerFamilies` in each `NodeFrameState` from the same projected graph node set, so preview/live/playback/export evaluators can consume layer semantics from one canonical runtime state instead of inferring from ad-hoc paths. Covered by `test/trueframe_core_runtime_evaluator_test.dart` and guard checks.
- TRUEFRAME Core Phase I parity coverage: projection/runtime tests now explicitly cover `groupPrecomp`, `sceneClipInstance`, and `adjustmentControl` families so non-transition layer-family stubs cannot silently regress while TrueFrame slices continue.
- TRUEFRAME Core Phase I backend routing contract: `TrueFrameRenderBackend` now exposes explicit node-family route decisions (`routeNodeFamilies`) with declared owners (`stage5Presenter`, `professionalCompositor`, `exportAdapter`, `blocked`) and explicit blockers for unsupported families. Covered by `test/trueframe_render_backend_test.dart`.
- TRUEFRAME Core Phase I explicit family-hint propagation: `MasterVisualSurface` now carries `coreLayerFamilyHint`, `MasterRenderGraphAdapter` emits that hint on graph nodes, and `TrueFrameExecutionGraphAdapter` resolves group/scene-clip/adjustment family expansion from the canonical hint only. Legacy target-name fallback has been removed and guarded by tests.
- TRUEFRAME Core Phase I screen/backend route integration: transition preview ownership in `FusionXCleanUiScreen` now uses `TrueFrameRenderBackend.routeNodeFamilies(...)` with resolved visual families from active transition clips, so UI routing consumes the generalized core backend contract instead of transition-only route calls.
- TRUEFRAME Core Phase I backend public API cleanup: transition-only route API is no longer exposed on `TrueFrameRenderBackend`; the public contract is now node-family based (`routeNodeFamilies(...)`) and transition ownership routing remains private implementation detail.
- TRUEFRAME manual temporal blur legacy cleanup: Manual Transition temporal sample generation no longer falls back to local `_stage5MotionBlurSampleOffsetsMs` sampling. Stage5 runtime samples and transition temporal plans now require `TrueFrameCoreRuntimeEvaluator.buildSamplingPlan(...)` as the only sample-time source. Guard tests now enforce removal of the old fallback branch.
...
- Composition Workspace W3 media permission correction: Android media import now checks image and video permissions independently, so an existing image/partial-visual grant no longer causes the Video tab to skip requesting video access and then show an empty device-video list. Partial visual access is remembered per media tab after a grant. This does not touch Stage5 or Live Scrub.
- Composition Workspace W3 nested media playback projection: `CompositionMediaPlaybackProjectionAdapter` now projects video/image layers inside Scene Scope and root Scene Clip containers into real media timeline tracks for native preview preparation and Live Scrub source descriptors. This fixes the hidden-media gap where a video layer existed inside a source composition but playback/scrub still saw only the root Scene Clip placeholder. Stage5 and Live Scrub internals remain untouched.
- Composition Workspace W3 Scene Contents native scrub/duration parity: Scene Contents now wires the native scrub surface when the open source composition contains playable video, maps root timeline time to scene-local native transport time and back, and preserves a video layer's natural asset duration by extending the source Scene Clip when needed. This is a Flutter adapter/timing integration; Stage5 and Live Scrub native internals remain untouched.
...
- Composition Workspace W3 video layer-scope entry: Scene video layers can now open the Unified Layer Scope authoring surface instead of being rejected as unsupported. Video scope exposes the shared visual property set (`position`, `scale`, `rotation`, `opacity`, and `blur`) as graph-backed channels, preserving video layer identity while keeping Stage5 and Live Scrub internals untouched. Deeper video renderer/export effect parity remains a tracked follow-up.
- Composition Workspace W3 video preview transform parity: the native preview surface is now wrapped by a graph-evaluated video transform surface so active Scene video layers visibly respond to authored `position`, `scale`, `rotation`, `opacity`, and `blur` samples during preview/scrub/playback. This is a Flutter preview bridge around the existing native surface; Stage5 and Live Scrub internals remain untouched, and production export parity for authored video surfaces remains a separate renderer checkpoint.
- Composition Workspace W3 scoped native scrub handoff: Scene Scope and Scene Layer Scope now route scrub start/end through the same native settle handoff used by the root timeline whenever the active composition scope contains playable video. Scoped Flutter code must not call `confirmScrubSettled` immediately while a native video surface is active; it must map root time to scene-local native transport time, wait for settle confirmation, and then reveal the player frame. Stage5 and Live Scrub native internals remain untouched.
- Composition Workspace W3 Scene Contents layer timing: Scene Contents layer clips can now be time-shifted with direct horizontal drag on the scoped timeline, writing back to the source `MotionLayerModel.visibleRange`, child element ranges, and related graph channel/keyframe times. Scene Contents rows must display in the same top-to-bottom draw order used by preview (`zIndex`, then insertion order), so the visually higher row is the higher-priority layer. Nested video playback projection now flattens overlapping video layers by z-index into the actually visible source intervals, so hidden time under a higher layer is not replayed later from source frame zero. Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W3 Scene Contents playback truth: authored composition time is now protected from the compact native media-program duration. Scene Contents video layers keep `layer.visibleRange` as the real source-scene placement, newly inserted media elements use layer-local ranges, active preview asset lookup maps root time to scene-local time before consulting scoped media rows, and empty timeline gaps remain visually blank instead of leaking the previous/next native frame. A native transport seek may clamp an authored gap to the compact media-program boundary only as an internal settle coordinate; it must never fall back to raw authored local time, because that can point into the wrong media clip. Composition canvas aspect is taken from the project format instead of imported video metadata. This is a Flutter adapter/timing correction only; Stage5 and protected Live Scrub internals remain untouched.
...
- Composition Workspace W7 Scene Contents transition bridge entry: adjacent video layers inside Scene Contents now render as joinable scene-video layer proxies and expose the same selectable bridge affordance used by the main timeline without reclassifying source-scene layers as root media clips. Bridge selections are stored per source scene as `TimelineTrackTransitionData`. The bridge browser now opens with `Preset`, then `Manual`, then `AI Transition`; `Preset` drills into the same bottom sheet with a back button and only currently engine-backed presets (`Cross Dissolve`, `Fade Black`). Applying a Scene Contents preset must return to the timeline immediately instead of stacking an inspector modal over playback/scrub. Full graph-backed manual transition scope, scene-video blending preview, and export parity remain explicit W7 follow-ups. This does not touch Stage5 or Live Scrub.
- Composition Workspace W7 Scene Contents media-program bridge stability: `TimelineMediaProgramTimeMapper` now owns the authored-time to compact-native-media-program mapping. Scene Contents transition previews can resolve scene-video layer proxies, carry source asset identity for thumbnails, and keep playback/scrub native-preview enabled even when the playhead is on an authored gap. Background deselection and bridge taps are ignored during scrub/handoff/structural edits so UI selection changes do not tear down native playback mid-gesture. This does not touch Stage5 or protected Live Scrub native internals.
- Composition Workspace W7 Scene Contents structural playback stability: Scene Contents video insertion and video-layer timing edits must use the same structural commit queue as root timeline media edits. The commit must serialize native transport preparation, projected-media scrub source priming, native scrub config flush, and readiness checks before user play/scrub can adopt new native samples. Full-interval projected media clips keep stable ids when a second clip is added, preventing Live Scrub store-key churn. This is Flutter adapter/transaction work only; Stage5 and protected Live Scrub native internals remain untouched.
- Composition Workspace W7 native preview identity stability: native preview identity must represent the active playback scope, not the currently active video clip. Scene Contents A/B video programs keep one stable `native-preview-scope:*` identity while clips are inserted or playback moves between them, preventing the Flutter fallback canvas from covering a still-valid Android video surface while native audio continues. This is Flutter preview identity work only; Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 root Scene transition preview parity: transitions authored inside Scene Contents must also preview when the root timeline plays the containing Scene Clip. The root preview path resolves the active root Scene Clip at the current playhead, maps root time into scene-local time, and reuses the same Scene Contents transition preview resolver instead of requiring the Scene Scope to be open. This is Flutter transition-preview projection work only; Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 Zoom In Camera professional gate: the previous Flutter-side Zoom In Camera preview path has been removed from the preset picker because it could only fake motion blur, transform a single native surface, and risk preview leakage outside the canvas. Existing saved Zoom transitions no longer draw fake speed lines or Gaussian transition blur. Zoom In Camera must return only after a real `ProfessionalVideoTransitionCompositor` provides dual-video sampling, temporal shutter motion blur, mirror-edge motion tiling, and preview/scrub/playback parity. Export parity remains a later release gate. This checkpoint does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 professional compositor timing foundation: `ProfessionalZoomCameraCompositorPlanner` now owns the first strict render-plan contract for a future native Zoom In Camera compositor. It maps transition timeline time to live outgoing/incoming source times, computes seam-aware zoom progress, requires mirror-edge tiling, and emits temporal shutter sample times from shutter angle/frame rate. `TransitionBrowserBottomSheet` now reads `kCurrentProfessionalVideoTransitionCompositorCapabilities`, so Zoom In Camera remains hidden until the real compositor reports full preview, Live Scrub, and playback parity. Export parity remains a later release gate. This is domain/UI-gate infrastructure only and does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 native compositor capability bridge: Android now exposes `com.refusion.app/professional_video_transition_compositor.getCapabilities`, and Flutter reads that native capability response before deciding whether Zoom In Camera may appear in the transition preset browser. The native response currently reports every professional Zoom requirement as unavailable, so the preset remains hidden until the real native compositor implements dual-video sampling, temporal motion blur, mirror-edge tiling, preview, scrub, and playback parity. Export parity remains a later release gate. This is a Flutter/native capability bridge only and does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 generic video transition render-plan contract: Flutter now serializes a generic `ProfessionalVideoTransitionRenderPlan` to Android through `prepareRenderPlan`, carrying transition definition id, canvas dimensions, seam timing, source ranges, required capabilities, parameters, sampling policy, edge policy, and motion blur policy. Zoom In Camera now lowers into that general contract instead of owning a private native shape. Android validates the request shape and deliberately returns an unsupported renderer status until concrete native renderers are implemented. This keeps unsupported transitions gated and prevents fallback to frozen frames, Gaussian blur, decorative speed lines, or transformed single-surface previews.
- Composition Workspace W7 video transition renderer registry foundation: Android now has a native renderer registry entry point for the general compositor. It reports known transition definitions and classifies `prepareRenderPlan` requests as unsupported definitions, missing renderer capabilities, or renderer not implemented. The registry currently rejects every definition until concrete native renderers are attached, preserving the no-fake transition rule.
- Composition Workspace W7 Cross Dissolve renderer planner foundation: `ProfessionalCrossDissolveCompositorPlanner` now owns the first primitive two-source blend timing contract. It evaluates normalized progress, outgoing/incoming opacity, and real outgoing/incoming source times from the generic render plan. It also marks whether both sources cover the full transition window; renderers must not enable cross dissolve when coverage is false because that would require frozen boundary frames. This is domain/test infrastructure only and does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 Cross Dissolve source-time bridge: Scene Contents video-layer proxies now preserve the underlying media source start/duration instead of using the layer's scene-local placement as media source time. Cross Dissolve boundary warmup therefore samples the outgoing layer's last visible media frame and the incoming layer's first visible media frame. The current Flutter preview bridge is seam-aware: before the seam it overlays the incoming first boundary frame over live outgoing playback; after the seam it overlays the outgoing last boundary frame over live incoming playback. This removes the jump to a late frame of B while the full native dual-video compositor remains the tracked renderer parity step. Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 Cross Dissolve clarity refinement: the built-in Cross Dissolve default is now a two-second symmetric handoff, the inspector allows up to five seconds, Scene Contents direct transition creation uses a linear alpha curve, and boundary-frame warmup requests canvas-resolution frames instead of low-resolution fallback thumbnails. This reduces visible pixel breakup in the interim preview bridge while preserving the strict rule that true 100% smooth Cross Dissolve requires the future dual-video native compositor.
- Composition Workspace W7 professional transition authoring gate: preset and AI video transition creation stay locked until `ProfessionalVideoTransitionCompositorCapabilities.canExposeProfessionalVideoTransitions` reports the complete interactive compositor stack: dual video sampling, temporal motion blur, mirror-edge tiling, preview parity, Live Scrub parity, and playback parity. Export parity is tracked by `canExportProfessionalVideoTransitions` and remains a later release gate. The `Manual` bridge entry is an authoring-scope exception: it may open the existing focused `TimelinePanel` transition timeline before the native compositor is complete so users can add/edit transition lanes and keyframes, but it must not claim final preview/playback/Live Scrub parity until the same compositor output path consumes those authored lanes. This prevents Cross Dissolve, Fade Black, Zoom In Camera, or AI transition drafts from becoming frozen-frame, Flutter-overlay, or single-surface fallbacks while still allowing manual transition timeline authoring. This checkpoint does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 manual transition timeline entry: bridge `Manual` now opens the focused transition authoring scope even while preset rendering is compositor-gated. The scope reuses the production `TimelinePanel` and its Animate/FX/Key/Value/Graph controls rather than creating a separate timeline widget. Root timeline transitions and Scene Contents source-scene transitions both enter the same focused authoring surface; Scene Contents manual lane edits are written back to the per-source-scene `TimelineTrackTransitionData` store instead of showing the old staged placeholder message. Scoped keyframe add/move operations now read the visible scoped playhead time from the timeline display notifier so added keys land on the frame the user is seeing instead of a stale root `_currentTime`. This does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 manual transition Animate/FX reuse: Manual Transition Scope now reuses the existing video layer-scope Animate and FX catalogs instead of the older transition-preset lane picker. `Animate` exposes the shared core controls (`Opacity`, `Position`, `Scale`, `Rotation`) and `FX` exposes the existing video `Gaussian Blur` control. Transition-specific legacy lanes remain readable for saved transitions, but new manual authoring starts from the same master motion vocabulary used by video layer scopes. This does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 native transition render-session foundation: Android `prepareRenderPlan` now parses a valid `ProfessionalVideoTransitionRenderPlan` into a strict two-source render session before consulting the renderer registry. The session validates canvas size, transition window, `[outgoing, incoming]` source roles, A/B source ranges, and whether outgoing reaches the boundary while incoming starts at the boundary. Unsupported responses now include `renderSessionId`, transition start/end, source roles, and boundary source times, giving the future GPU compositor a concrete session contract instead of a loose map. This still reports missing capabilities and renders nothing, preserving the no-transition-before-compositor-complete rule and avoiding Stage5/Live Scrub changes.
- Composition Workspace W7 native transition source-binding contract: `ProfessionalVideoTransitionRenderPlan` sources can now carry concrete `sourceUri` values, and Android exposes `planVideoSourceBindings` to validate outgoing/incoming video source binding before exact decode can claim readiness. `assetId` remains identity metadata only; source URI absence blocks the decoder path with `native_video_source_uri_missing`, and asset-id-only or generated-proxy decode remains forbidden.
- Composition Workspace W7 source-bound render-plan adapter: `ProfessionalVideoTransitionRenderPlanAdapter` converts adjacent `TimelineClipData` pairs and a `sourceUri` resolver into the strict two-source compositor render plan. It fails closed when a source URI is missing, when a clip lacks visible/source handles for the requested leading/trailing window, or when speed-overridden clips would require source-rate mapping the compositor contract does not yet carry. This is production-facing adapter infrastructure only; it does not expose any video transition preset while the native compositor capabilities remain unavailable.
- Composition Workspace W7 professional transition readiness preflight: `ProfessionalVideoTransitionReadinessPreflight` now runs the complete interactive compositor-readiness chain from capabilities through render session, source binding, frame samples, exact decode, dual decoder, temporal accumulation, mirror-edge tiling, render-pass graph, graph execution, output surface, surface renderer, frame render commands, renderer backend, renderer draw loop, shader evaluation, pixel workload, pixel render execution, native pixel output proof, and preview/scrub/playback parity. It reports the exact blocking stages instead of allowing UI or future agents to infer readiness from one partial endpoint. Export is explicitly deferred to a later release gate. This remains domain infrastructure and does not unlock any transition preset.
- Composition Workspace W7 transition readiness presentation: `ProfessionalVideoTransitionReadinessPresentationAdapter` now turns capability and preflight reports into a stable UI blocker model, and `TransitionBrowserBottomSheet` displays ordered readiness stages instead of a generic missing-capability string. Transition UI remains locked until the same professional compositor readiness model reports no blockers; this is presentation-only and does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 native transition source-probe implementation: Flutter and Android now share `planVideoSourceProbe` as the required bridge between source URI binding and exact frame decode. Android uses `MediaExtractor` against `file://` and `content://` sources to prove each bound URI is openable and contains a real video track before exact decode can advance. The probe reports video metadata such as MIME type, width, height, duration, and frame rate, and blocks missing, unsupported, permission-denied, non-video, synthetic, asset-id-only, generated-proxy, thumbnail, and boundary-frame fallback paths. This still does not unlock transitions because the dual decoder, temporal accumulator, mirror-edge tiler, output surface, and parity renderer remain explicit follow-up stages.
- Composition Workspace W7 native transition frame-sample contract: Android now exposes `planFrameSamples` on the professional video transition compositor channel. It reuses the strict render-session parser, rejects timeline times outside the transition window, maps the requested timeline frame to real outgoing/incoming source times, and emits temporal shutter sample timeline/source times from the render plan's motion blur policy. This is a renderer contract only: it still draws nothing and exposes no transition preset until native preview, Live Scrub, and playback parity are complete.
- Composition Workspace W7 native transition decode-request contract: Android now exposes `planFrameDecodeRequests` on the same compositor channel. It turns the strict frame-sample plan into per-source exact video decode requests with role, clip id, asset id, timeline sample time, source sample time, sample index, and center-sample metadata. The contract explicitly forbids thumbnail fallback and boundary-frame freezing. It is still a planning layer only and does not unlock transition rendering.
- Composition Workspace W7 native dual-video decoder readiness: Android now exposes `planDualVideoDecoderSession` on the professional compositor channel. It groups exact outgoing/incoming decode requests into two video decoder tracks, preserves sample ids and source asset identity, requires successful real source probing, and performs a strict center-sample `MediaCodec` decode probe for each side before the decoder stage can advance. Thumbnail fallback and boundary-frame freeze remain explicitly forbidden. This proves dual-video sampling readiness but still does not unlock professional transitions until temporal accumulation, mirror-edge tiling, output surface, and preview/playback/Live Scrub parity are implemented.
- Composition Workspace W7 native decoded-buffer gate: exact video decode readiness now requires readable decoded `MediaCodec` output buffers, not only decoded presentation timestamps. Android records decoded buffer counts, center-sample byte counts, and deterministic buffer checksums for outgoing/incoming shutter samples, and blocks with `native_exact_frame_output_buffer_not_ready` or `native_temporal_sample_buffer_not_ready` when decoded sample pixels are not proven readable. This prevents future motion blur or dual-video transitions from being unlocked from metadata, poster frames, or frozen boundary stills. Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 native temporal sample accumulator readiness: Android now exposes `planTemporalSampleAccumulator` on the professional compositor channel. It binds the dual-video decoder tracks into outgoing/incoming temporal accumulators with deterministic sample weights, decoded shutter-sample counts, exact-frame requirements, and explicit `allowGaussianFallback=false` / `allowDecorativeSpeedLines=false`. The accumulator now blocks if any shutter sample from either side failed the native decode probe, and still remains blocked with `accumulatorImplemented=false` until real temporal shutter accumulation exists, so transitions stay locked and no fake blur path can return.
- Composition Workspace W7 native mirror-edge tiling readiness: Android now exposes `planMirrorEdgeTiling` on the professional compositor channel. It binds temporal accumulator outputs into outgoing/incoming mirror-edge tile plans with deterministic overscan, decoded-sample readiness, canvas clipping, and explicit `allowBlackBorders=false`, `allowFlutterOverlay=false`, and `allowTimelineOverlay=false`. The tiler blocks if temporal input samples are not decodable, and remains blocked with `tilerImplemented=false` until a real native mirror-edge tiler exists, preventing black border, stretched thumbnail, or timeline-overlay substitutes.
- Composition Workspace W7 native transition render-pass graph readiness: Android now exposes `planRenderPassGraph` on the professional compositor channel. It converts exact video decode requests into a renderer-agnostic pass graph: exact decode, temporal sample accumulation, optional mirror-edge tile, transition shader evaluation, and composition to the transition output surface. The graph now propagates upstream blockers from decode, temporal accumulation, and mirror-edge tiling, then reports `rendererImplemented=false`, so it still cannot render or expose presets until the real native compositor is complete.
- Composition Workspace W7 native transition output-surface readiness: Android now exposes `planOutputSurface` on the professional compositor channel. It binds the pass graph to a canvas-clipped native transition output target, explicitly forbids Flutter overlays, timeline overlays, and PlatformView-transform fallbacks, and inherits upstream render blockers. The output surface remains blocked with `rendererImplemented=false` until a concrete native renderer owns preview, Live Scrub, and playback parity. Export remains a separate later phase.
- Composition Workspace W7 native transition parity-output contract: Android now exposes `planParityOutputs` on the professional compositor channel. It requires preview, Live Scrub, and playback to share the same native transition output contract and keeps those interactive modes blocked until the concrete renderer can render without fallback divergence. Export remains a later phase and must join the same output contract when built.
- Composition Workspace W7 Zoom In Pro experiment closed: the temporary `Zoom In Pro` live-surface experiment is no longer exposed in the transition preset picker. A transformed single native preview surface cannot deliver dual-video sampling, temporal shutter motion blur, mirror-edge tiling, or stable preview/Live Scrub/playback parity. The final professional path remains the native dual-video compositor with temporal shutter motion blur, mirror-edge tiling, preview/Live Scrub/playback parity, and later export parity.
- Composition Workspace W7 native accumulator/tiler readiness advancement: Android temporal accumulation now advances from a pure missing-stage placeholder to a decoded-buffer-backed readiness stage. It requires outgoing and incoming shutter samples to be exactly decodable with readable decoded output buffers, records accumulated byte counts/checksums, and reports `accumulatorImplemented=true` only when both roles have complete decoded sample coverage. Mirror-edge tiling now consumes those accumulator readiness signals and reports tile readiness/overscan without allowing black borders, Flutter overlays, timeline overlays, or thumbnail substitutes. The concrete transition renderer and output surface still remain blocked until a native renderer can draw the graph for preview, Live Scrub, and playback parity.
- Composition Workspace W7 native live-decode stream gate: Android dual-video decoder planning now separates exact sample decode probes from continuous video-stream readiness. Each outgoing/incoming decoder track records the required live decode timeline/source window, requires outgoing to cover the leading seam window and incoming to cover the trailing seam window, probes start/mid/end source samples with readable decoded buffers, and now performs a sequential `MediaCodec` decode stream probe across the live window. `continuousSampleCoverageReady` requires both exact coverage samples and the sequential stream probe, so a future renderer cannot animate one decoded still frame while claiming video motion. It blocks with `native_dual_video_live_decode_not_ready` and `native_dual_video_live_decode_stream_not_ready` until both roles prove coverage. Presets remain locked until the downstream temporal accumulator, mirror-edge tiler, renderer, output surface, and preview/Live Scrub/playback parity stages render real pixels.
- Composition Workspace W7 temporal accumulator stream dependency: Android temporal accumulator readiness now inherits the dual decoder's live-stream proof. Accumulators carry live decode window/stream counts and require `liveDecodeStreamCoverageReady` plus `continuousSampleCoverageReady` before an accumulated frame can be marked ready. This prevents a future motion-blur stage from accumulating exact samples while ignoring that the underlying video stream cannot play across the transition window. The stage now blocks with `native_temporal_live_decode_stream_not_ready` when stream readiness is missing.
- Composition Workspace W7 mirror-edge stream dependency: Android mirror-edge tiling readiness now inherits temporal accumulator live-stream readiness. Tile plans carry `liveDecodeStreamCoverageReady` and `continuousSampleCoverageReady`, and block with `native_mirror_edge_live_decode_stream_not_ready` when a transition would otherwise tile/overscan frames from a non-playing source window. This keeps the no-black-border path attached to real moving video, not still samples.
- Composition Workspace W7 live-stream render-pass graph: Android render-pass graph planning now models live stream decode as an explicit `decodeLiveVideoStreams` pass before exact frame decode, temporal accumulation, mirror-edge tiling, shader evaluation, and output composition. Temporal passes now depend on this live stream pass plus exact decode request ids, so the renderer graph cannot silently omit the moving-video dependency while still claiming motion blur readiness.
- Composition Workspace W7 output-surface graph binding: Android output-surface planning now binds the native transition canvas surface to the final `composeToTransitionSurface` render pass. The plan records render pass count, output pass id/type/inputs, `outputPassBound`, and `renderGraphOutputReady`, and blocks with `native_transition_output_pass_missing` if the graph has no valid output pass. This prevents future renderers from drawing through side surfaces, Flutter overlays, timeline overlays, or PlatformView transforms outside the real compositor graph.
- Composition Workspace W7 parity-output graph binding: Android parity planning now carries the same output pass binding into `preview`, `liveScrub`, and `playback` outputs. Each mode records output pass id/type/inputs, `outputPassBound`, `renderGraphOutputReady`, pixel output proof id, `outputProofReady`, upload-packet readiness, and final surface endpoint state; mode-level `canRender` requires that binding, pixel output proof, an attached endpoint, and a concrete renderer. This prevents a future transition from claiming parity through matching surface ids while one mode bypasses the final compositor pass or while the real native transition endpoint is still missing.
- Composition Workspace W7 native render-graph executor foundation: Android now exposes `planRenderGraphExecution` on the professional compositor channel. The executor validates the ordered pass sequence (`decodeLiveVideoStreams`, exact decode, temporal accumulators, mirror-edge tile when required, transition shader, and `composeToTransitionSurface`), verifies pass dependencies point backward through the graph, reports pass execution states, and proves graph ownership without drawing pixels. It keeps `rendererImplemented=false` and blocks execution with `native_transition_render_graph_executor_renderer_missing` until a concrete native renderer attaches.
- Composition Workspace W7 native surface-renderer skeleton: Android now exposes `planSurfaceRenderer` on the professional compositor channel and Flutter includes it in transition readiness preflight between output-surface binding and parity outputs. The skeleton attaches the ordered render graph to the canvas-clipped native transition output surface and final `composeToTransitionSurface` pass, but still reports `rendererImplemented=false`, `rendersRealPixels=false`, and `drawsPixels=false`. It blocks with `native_transition_surface_renderer_pixels_missing` until a concrete native renderer draws real transition pixels, preventing agents or UI from treating an attached surface as a renderable transition.
- Composition Workspace W7 native frame-render command buffer: Android now exposes `planFrameRenderCommands` on the professional compositor channel and Flutter includes it in readiness preflight after the surface-renderer skeleton. This stage lowers the ordered render graph into per-frame native render commands for intermediate buffers and the final `nativeTransitionCanvasSurface`, preserving pass id/type/role/input dependencies and requiring real pixels. It reports `rendererCommandBufferImplemented=true` and a complete command graph, but still blocks with `native_transition_frame_command_renderer_missing` until the concrete native renderer can submit commands and draw real pixels.
- Composition Workspace W7 native renderer backend gate: Android now exposes `planRendererBackend` on the professional compositor channel and Flutter includes it in readiness preflight after frame render commands. This gate proves the native backend path can see GPU/OpenGL ES capability, command-buffer readiness, canvas-surface attachment, and the final output target, but it deliberately keeps `drawLoopImplemented=false`, `rendererImplemented=false`, `rendersRealPixels=false`, and `drawsPixels=false`. It blocks with `native_transition_renderer_draw_loop_missing` and `native_transition_renderer_pixels_missing` until a concrete native draw loop submits commands and writes real transition pixels. This does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 native renderer draw-loop submission contract: Android now exposes `planRendererDrawLoop` on the professional compositor channel and Flutter includes it in readiness preflight after the renderer backend gate. The renderer backend stage now proves backend attachment separately, while the draw-loop stage turns frame render commands into ordered draw submissions for the final `nativeTransitionCanvasSurface`. It reports `drawLoopImplemented=true` and `canSubmitCommands=true` when the backend and command buffer are structurally ready, but still reports `canRenderFrame=false`, `rendersRealPixels=false`, and `drawsPixels=false`. Shader evaluation and pixel rendering are separate downstream gates, so command submission cannot be treated as a visual transition. This keeps Stage5/Live Scrub untouched.
- Composition Workspace W7 native transition shader evaluation gate: Android now exposes `planTransitionShaderEvaluation` on the professional compositor channel and Flutter includes it in readiness preflight after the renderer draw-loop gate. The shader gate converts ordered draw submissions into explicit shader inputs, preserves temporal-shutter and mirror-edge requirements, and can advance only when the draw loop and shader inputs are bound. It deliberately still reports `canRenderFrame=false`, `rendersRealPixels=false`, and `drawsPixels=false`; the next stage is a concrete native pixel renderer that writes real transition pixels. This does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 native transition pixel workload gate: Android now exposes `planTransitionPixelRenderer` on the professional compositor channel and Flutter includes it in readiness preflight after shader evaluation. The pixel gate binds shader inputs into a pixel workload for `nativeTransitionCanvasSurface` without claiming rendered pixels. Android also exposes `planTransitionPixelFrameBuffer`, which requires a native canvas-sized `rgba8888` frame buffer, allocates bounded `DirectByteBuffer` storage for that buffer, and forbids synthetic pixels/poster frames/thumbnails/boundary freezes. Android also exposes `planTransitionPixelFrameBufferWriter`, which binds a native writer to that allocated buffer, requires temporal dual-source samples, forbids still-frame writes, extracts real outgoing/incoming video frames at shutter sample times with `MediaMetadataRetriever.getFrameAtTime`, center-crop-fills them into the canvas-sized buffer, and reports sample count, extracted-frame count, write byte count, checksum, extractor, and fill mode. Android also exposes `planTransitionPixelRenderExecution`, which consumes the writer output into an offscreen native pixel-output frame and reports source frame-buffer id, write mode, byte count, checksum, and reason. Pixel output proof now builds a native surface-upload packet contract from that offscreen frame, with upload packet id, source buffer id, byte count, checksum, and reason. This prevents shader-ready, workload-ready, memory-ready, source-pixel-ready, offscreen-pixel-ready, or upload-packet-ready plans from unlocking presets without true native transition output, and it does not touch Stage5 or protected Live Scrub internals.
- Composition Workspace W7 transition surface endpoint preflight stage: `ProfessionalVideoTransitionReadinessPreflight` now exposes `transitionSurfaceEndpoint` as an explicit stage between pixel output proof and parity outputs. It advances only when a real upload packet exists, the surface-upload renderer is ready, and the final native transition endpoint is attached; otherwise it reports blockers such as `native_transition_surface_endpoint_missing`. This keeps the final interactive blocker visible without exposing any transition preset.
- Composition Workspace W7 native transition surface endpoint proof: Android now owns a bounded offscreen `ImageReader`/`Surface` endpoint store for native transition output proof. Pixel output proof copies the canvas-sized `rgba8888` frame buffer into that native endpoint, records endpoint byte count/checksum, and can mark the proof endpoint attached when the surface accepts the posted frame. This is still an offscreen proof path only; transition presets remain locked until preview, Live Scrub, and playback parity consume the same compositor output through their real interactive surfaces. Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 interactive transition surface contract: parity planning now explicitly separates the offscreen proof endpoint (`offscreenNativeProofSurface`) from the real interactive surfaces required by `preview`, `liveScrub`, and `playback`. Each parity mode must report an `interactiveNativeTransitionSurface` binding before it can render. Until those bindings exist, parity reports `native_transition_<mode>_interactive_surface_missing`, even if the offscreen endpoint proof succeeds. This prevents a proof `ImageReader`, Flutter overlay, timeline overlay, or transformed PlatformView from being mistaken for the professional transition output.
- Composition Workspace W7 interactive transition frame delivery: parity planning now pushes the source-derived transition frame buffer into separate per-mode interactive native endpoints for `preview`, `liveScrub`, and `playback`. Each mode records delivered frame state, byte count, checksum, and reason. A mode remains blocked with `native_transition_<mode>_interactive_surface_frame_missing` unless its own `interactiveNativeTransitionSurface` accepts a real frame packet; the offscreen proof endpoint alone still cannot unlock a preset or fallback preview.
- Composition Workspace W7 interactive transition frame presentation proof: interactive parity now distinguishes accepted frame delivery from presented/acquired native output. Each preview, Live Scrub, and playback endpoint must present an acquired `ImageReader` frame with image count, byte count, checksum, and reason before parity can render. Missing presentation reports `native_transition_<mode>_interactive_surface_presentation_missing`, keeping transition presets locked when a surface accepted an upload but did not actually expose a consumable frame.
- Composition Workspace W7 interactive production surface gate: accepted/presented proof frames still cannot unlock a transition. The current `ImageReader` path must identify itself as `interactiveNativePresentationProofSurface`, not as a production `interactiveNativeTransitionSurface`. Preview, Live Scrub, and playback parity remain blocked with `native_transition_<mode>_production_surface_missing` until each mode binds a real production transition surface and presents the compositor output there.
- Composition Workspace W7 explicit interactive surface binding: `ProfessionalVideoTransitionRenderPlan` now carries optional `interactiveSurfaceBindings` for `preview`, `liveScrub`, and `playback`, and the source-bound render-plan adapter preserves those bindings into the native method-channel payload. Android parity only treats a mode as production-bound when the matching binding is attached, has a stable surface id, and declares `interactiveNativeTransitionSurface`; otherwise it falls back to proof-surface diagnostics and stays blocked.
- Composition Workspace W7 real interactive transition surface binding: Flutter now owns a dedicated Android `ProfessionalVideoTransitionSurface` PlatformView for transition output, Android registers that visible `Surface` with `ProfessionalVideoTransitionCompositorManager`, and `renderInteractiveFrame` draws the source-derived transition frame buffer into that registered surface for preview, Live Scrub, and playback modes. Bound interactive uploads are now strict: a surface id that was not registered by the real PlatformView returns `native_transition_interactive_surface_not_registered` instead of allocating an offscreen proof endpoint. Export remains intentionally gated for a later phase.
- Composition Workspace W7 Manual Transition scope ownership correction: Manual Transition Focus now separates the wide editor scope from the render-active seam window. A manual transition with authored lanes is considered visually active only inside its real leading/trailing transition window, so the native video preview surface is not suppressed across the surrounding authoring context. Manual `manualTransform` continues through the dedicated interactive transition surface only when the playhead is actually inside the transition window. Stage5 and protected Live Scrub internals remain untouched.
- Composition Workspace W7 Manual Transition Stage5 visual runtime bridge: Manual Transition animate keyframes now emit a dedicated `Stage5VisualRuntimeState` from the Master Clock/Value Truth path (manual lane adapter -> master evaluation -> live scrub visual program), and that runtime state is submitted through `Stage5NativeTransportController` into `Stage5NativeScrubEngine` for frame-time visual ownership. Runtime transform/opacity state is applied to the active scrub overlay host and to the visible Stage5 player surface path, so manual keyframe edits no longer require legacy `ProfessionalVideoTransitionSurfaceOverlay` rendering to show authored motion.
- Composition Workspace W7 Manual Transition runtime stability follow-up: manual Stage5 runtime evaluation now resolves against the effective motion project and explicit preview-time clock override so keyframe value edits do not read stale frame-time snapshots. Add-key in Manual Transition scope now requires the playhead to be inside the active transition window (no silent clamp to edge), and native timeline scrub config updates now allow runtime visual source pushes during active scrub sessions so authored transform changes can propagate immediately to Stage5.
- Composition Workspace W7 transition apply preflight diagnostics: zoom-family professional presets now run a render-plan preflight before they are staged from root or Scene Contents transition bridges. Missing source URIs, unresolved A/B boundaries, short source handles, or unbound native definitions block the apply action with an explicit stage message instead of creating a silent transition that falls back to an empty preview. Preview-time render-plan failures are logged once per transition/mode. This does not add Flutter fake zoom fallback and does not touch Stage5 or protected Live Scrub internals.
- Professional Canva layer unification plan: `docs/professional_canva_layer_unification_plan.md` is now the official plan for closing the remaining Manual Transition Scale/Animate gap by making video resolve to the same composition-layer truth as image/text/shape. The plan adds `Keyframe Time Truth`, `Video Layer Truth`, unified target resolution, Manual Transition as composition scope, shared Visual Layer Program, Stage5 renderer adapter boundaries, FX ordering, and legacy manual renderer guards. This supersedes treating Manual Transition Scale as a standalone fix.
- Composition Workspace W9 authored video export contract: authored video/image/shape/mask surfaces now expose their exact kinds through `SceneExportParityGate`, and export tests prove video layer surfaces are included in the authored visual surface program with graph channels and source asset identity. Production export remains blocked until the native authored visual renderer ships, but the blocker now names video explicitly instead of hiding it behind a generic shape/image warning.
...
- Manual Transition follows the same safety boundary while it is backed by the
  current per-request frame extraction bridge. Its wide focus timeline is an
  editor scope only; native transition rendering and preview-surface ownership
  may activate only inside the real leading/trailing seam window.
