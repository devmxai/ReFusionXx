...
- Composition Workspace W7 professional transition readiness preflight: `ProfessionalVideoTransitionReadinessPreflight` now runs the complete interactive compositor-readiness chain from capabilities through render session, source binding, frame samples, exact decode, dual decoder, temporal accumulation, mirror-edge tiling, render-pass graph, output surface, and preview/scrub/playback parity. It reports the exact blocking stages instead of allowing UI or future agents to infer readiness from one partial endpoint. Export is explicitly deferred to a later release gate. This remains domain infrastructure and does not unlock any transition preset.
