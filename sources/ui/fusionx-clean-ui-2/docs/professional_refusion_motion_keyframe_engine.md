...
- Composition Workspace W7 native zoom render-plan contract: Flutter now serializes a complete `ProfessionalZoomCameraRenderPlan` to Android through `prepareZoomInCameraRenderPlan`, carrying canvas dimensions, seam timing, outgoing/incoming source ranges, shutter settings, and mirror-edge tile overscan. Android validates the request shape and deliberately returns an unsupported renderer status until the real dual-video GPU compositor is implemented. This keeps Zoom In Camera gated and prevents any fallback to frozen frames, Gaussian blur, decorative speed lines, or transformed single-surface previews.
