...
- Composition Workspace W7 native transition source-probe contract: Flutter and Android now share `planVideoSourceProbe` as the required bridge between source URI binding and exact frame decode. The preflight chain now blocks on `sourceMediaProbe` until native proves each bound URI is a real openable video source with a video track; synthetic sources, asset-id-only decode, generated proxies, and skipping the probe are forbidden. The current Android foundation deliberately reports `probeImplemented=false`, so transitions remain locked without falling back to thumbnails or frozen boundary frames.
