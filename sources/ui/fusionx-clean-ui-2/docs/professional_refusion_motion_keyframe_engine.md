...
- Composition Workspace W7 Scene Contents transition bridge entry: adjacent video layers inside Scene Contents now render as joinable scene-video media clips and expose the same selectable bridge affordance used by the main timeline. Bridge selections are stored per source scene as `TimelineTrackTransitionData` and can open the existing transition browser/inspector entry point for preset, manual, or AI transition intent. Full graph-backed manual transition scope, scene-video blending preview, and export parity remain explicit W7 follow-ups. This does not touch Stage5 or Live Scrub.
