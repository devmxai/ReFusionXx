...
- Composition Workspace W7 native transition pixel renderer gate: Android now exposes `planTransitionPixelRenderer` on the professional compositor channel and Flutter includes it in readiness preflight after shader evaluation. The pixel gate binds shader inputs into a pixel workload for `nativeTransitionCanvasSurface` and keeps transitions blocked with `native_transition_pixel_renderer_missing` / `native_transition_renderer_pixels_missing` until a concrete native renderer writes real pixels. This prevents shader-ready or command-ready plans from unlocking presets without a true pixel renderer, and it does not touch Stage5 or protected Live Scrub internals.
