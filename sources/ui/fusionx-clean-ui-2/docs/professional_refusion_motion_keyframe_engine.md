...
- Composition Workspace W7 native transition pixel workload gate: Android now exposes `planTransitionPixelRenderer` on the professional compositor channel and Flutter includes it in readiness preflight after shader evaluation. The pixel gate binds shader inputs into a pixel workload for `nativeTransitionCanvasSurface` without claiming rendered pixels. Android also exposes `planTransitionPixelFrameBuffer`, which requires a native canvas-sized `rgba8888` frame buffer, allocates bounded `DirectByteBuffer` storage for that buffer, and forbids synthetic pixels/poster frames/thumbnails/boundary freezes. Android also exposes `planTransitionPixelFrameBufferWriter`, which binds a future native writer to that allocated buffer, requires temporal dual-source samples, forbids still-frame writes, and keeps transitions blocked with `native_transition_pixel_frame_buffer_writer_missing`, `native_transition_pixel_frame_buffer_temporal_pixels_missing`, and `native_transition_pixel_frame_buffer_pixels_missing` until real moving video pixels are written into the buffer. Android also exposes `planTransitionPixelRenderExecution`, which now depends on the writer gate before binding the workload to the native output framebuffer and keeps transitions blocked with pixel-writer, pixel-output, and renderer-pixel blockers until a concrete native renderer writes real pixels. This prevents shader-ready, workload-ready, or memory-ready plans from unlocking presets without true temporal pixel output, and it does not touch Stage5 or protected Live Scrub internals.
