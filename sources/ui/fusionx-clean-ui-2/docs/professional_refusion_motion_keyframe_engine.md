...
- Composition Workspace W7 professional transition authoring gate: new video transition authoring is now locked until `ProfessionalVideoTransitionCompositorCapabilities.canExposeProfessionalVideoTransitions` reports the complete interactive compositor stack: dual video sampling, temporal motion blur, mirror-edge tiling, preview parity, Live Scrub parity, and playback parity. Export parity is tracked by `canExportProfessionalVideoTransitions` and remains a later release gate. The transition bridge still shows the planned `Preset`, `Manual`, and `AI Transition` entries, but they are disabled with an explicit compositor-readiness blocker while Android reports the foundation/unavailable capability set. This prevents Cross Dissolve, Fade Black, Zoom In Camera, manual transition lanes, or AI transition drafts from being created as frozen-frame, Flutter-overlay, or single-surface fallbacks. This checkpoint does not touch Stage5 or protected Live Scrub internals.
