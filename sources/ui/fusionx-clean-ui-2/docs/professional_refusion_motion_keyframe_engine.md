...
- Composition Workspace W7 native mirror-edge tiling contract: Android now exposes `planMirrorEdgeTiling` on the professional compositor channel. It binds temporal accumulator outputs into outgoing/incoming mirror-edge tile plans with deterministic overscan, canvas clipping, and explicit `allowBlackBorders=false`, `allowFlutterOverlay=false`, and `allowTimelineOverlay=false`. The tiler remains blocked with `tilerImplemented=false` until a real native mirror-edge tiler exists, preventing black border, stretched thumbnail, or timeline-overlay substitutes.
