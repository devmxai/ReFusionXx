...
- Composition Workspace W3 Scene Contents Add refinement: the Scene Scope bottom `Add` command now opens the context-aware layer insertion sheet for the open source scene. Video/image media import and text/shape creation are available from the same Scene Contents Add gateway, while audio/null/adjustment remain visible planned blockers. This keeps Add scoped to source-scene layers rather than root media insertion or layer-scope keyframes, and does not touch Stage5 or Live Scrub.
