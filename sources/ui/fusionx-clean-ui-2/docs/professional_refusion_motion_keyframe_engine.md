...
- Composition Workspace W7 Zoom In Camera live-surface preview: the Zoom In Camera preset now drives the native video preview surface itself over a 4s default two-sided window instead of covering playback with frozen boundary-frame JPEGs. Exact boundary-frame extraction remains available for AI seeds and fallback transitions, but live Zoom preview avoids generic thumbnail fallback and keeps Stage5/Live Scrub internals untouched. True directional native motion blur and export parity remain future renderer work.
