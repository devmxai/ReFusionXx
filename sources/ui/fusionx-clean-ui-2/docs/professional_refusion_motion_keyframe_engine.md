...
- Composition Workspace W7 native dual-video decoder session contract: Android now exposes `planDualVideoDecoderSession` on the professional compositor channel. It groups exact outgoing/incoming decode requests into two video decoder tracks, preserves sample ids and source asset identity, and keeps the session blocked with `decoderImplemented=false` until a real native dual-video decoder exists. Thumbnail fallback and boundary-frame freeze remain explicitly forbidden.
