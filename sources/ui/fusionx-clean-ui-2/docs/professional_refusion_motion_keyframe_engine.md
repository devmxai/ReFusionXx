...
- Composition Workspace W7 native temporal sample accumulator contract: Android now exposes `planTemporalSampleAccumulator` on the professional compositor channel. It binds the dual-video decoder tracks into outgoing/incoming temporal accumulators with deterministic sample weights, exact-frame requirements, and explicit `allowGaussianFallback=false` / `allowDecorativeSpeedLines=false`. The accumulator remains blocked with `accumulatorImplemented=false` until real temporal shutter accumulation exists, so transitions stay locked and no fake blur path can return.
