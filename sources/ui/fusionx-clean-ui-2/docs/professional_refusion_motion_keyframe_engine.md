...
- Composition Workspace W7 parity-output graph binding: Android parity planning now carries the same output pass binding into `preview`, `liveScrub`, and `playback` outputs. Each mode records output pass id/type/inputs, `outputPassBound`, and `renderGraphOutputReady`; mode-level `canRender` requires that binding plus a concrete renderer. This prevents a future transition from claiming parity through matching surface ids while one mode bypasses the final compositor pass.
