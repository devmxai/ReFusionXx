...
- Composition Workspace W7 native transition pixel workload gate: Android now exposes `planTransitionPixelRenderer` on the professional compositor channel and Flutter includes it in readiness preflight after shader evaluation. The pixel gate binds shader inputs into a pixel workload for `nativeTransitionCanvasSurface` without claiming rendered pixels. Android also exposes `planTransitionPixelFrameBuffer`, which requires a native canvas-sized `rgba8888` frame buffer, forbids synthetic pixels/poster frames/thumbnails/boundary freezes, and blocks with `native_transition_pixel_frame_buffer_missing` and `native_transition_pixel_frame_buffer_pixels_missing` until a concrete renderer fills it with real temporal video pixels. Android also exposes `planTransitionPixelRenderExecution`, which binds that workload to the native output framebuffer and keeps transitions blocked with `native_transition_pixel_renderer_missing`, `native_transition_pixel_output_missing`, and `native_transition_renderer_pixels_missing` until a concrete native renderer writes real pixels. This prevents shader-ready or workload-ready plans from unlocking presets without true pixel output, and it does not touch Stage5 or protected Live Scrub internals.
