...
- Composition Workspace W7 native transition render-pass graph contract: Android now exposes `planRenderPassGraph` on the professional compositor channel. It converts exact video decode requests into a renderer-agnostic pass graph: exact decode, temporal sample accumulation, optional mirror-edge tile, transition shader evaluation, and composition to the transition output surface. The graph reports `rendererImplemented=false`, so it still cannot render or expose presets until the real native compositor is complete.
- Composition Workspace W7 native transition output-surface contract: Android now exposes `planOutputSurface` on the professional compositor channel. It binds the pass graph to a canvas-clipped native transition output target and explicitly forbids Flutter overlays, timeline overlays, and PlatformView-transform fallbacks. The output surface remains blocked with `rendererImplemented=false` until a concrete native renderer owns preview, Live Scrub, playback, and export parity.
- Composition Workspace W7 native transition parity-output contract: Android now exposes `planParityOutputs` on the professional compositor channel. It requires preview, Live Scrub, playback, and export to share the same native transition output contract and keeps every mode blocked until the concrete renderer can render all four modes without fallback divergence.
