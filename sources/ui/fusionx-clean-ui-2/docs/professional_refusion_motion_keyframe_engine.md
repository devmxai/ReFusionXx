...
- Composition Workspace W3 Scene Contents layer timing: Scene Contents layer clips can now be time-shifted directly on the scoped timeline, writing back to the source `MotionLayerModel.visibleRange`, child element ranges, and related graph channel/keyframe times. Nested video playback projection now flattens overlapping video layers by z-index into the actually visible source intervals, so hidden time under a higher layer is not replayed later from source frame zero. Stage5 and protected Live Scrub internals remain untouched.
